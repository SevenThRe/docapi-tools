...
def test_resolve_install_spec_prefers_local_wheel_for_local_manifest(tmp_path: Path) -> None:
    from scripts.release_tools import resolve_install_spec

    manifest_path = tmp_path / "release-manifest.json"
    wheel_path = tmp_path / "docapi_tools-0.1.4-py3-none-any.whl"
    wheel_path.write_bytes(b"wheel")
    manifest = {
        "version": "0.1.4",
        "wheel": wheel_path.name,
        "install_spec": "https://example.com/docapi/v0.1.4/docapi_tools-0.1.4-py3-none-any.whl",
    }
    manifest_path.write_text(json.dumps(manifest, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    resolved = resolve_install_spec(manifest, manifest_source=str(manifest_path))
    assert resolved == str(wheel_path.resolve())
...
def test_build_release_artifacts_writes_manifest_and_scripts(monkeypatch, tmp_path: Path) -> None:
    def fake_run(command, check, capture_output, text):  # noqa: ANN001
        ...
        (output_dir / "docapi_tools-0.1.4-py3-none-any.whl").write_bytes(b"wheel-bytes")
    ...
    report = build_release_artifacts(
        ...
        base_url="https://example.com/docapi/v0.1.4",
    )
    ...
    assert manifest["install_spec"] == "https://example.com/docapi/v0.1.4/docapi_tools-0.1.4-py3-none-any.whl"
    ...
    assert "$PSScriptRoot" in install_script
    assert "../runtime/Scripts/docapi.exe" in install_script
